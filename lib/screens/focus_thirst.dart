...
class FocusThirst extends StatefulWidget {
  final File? image;
  final double width;
  final double heigth;
  ...
  @override
  _FocusThirstState createState() => _FocusThirstState();
}

class _FocusThirstState extends State<FocusThirst> {
  List<Point> points = [];
  ...
  double calculateDistance() {
    double value;
    if (widget.selectedReference == 'Kart/Kimlik') {
      value = 8.56;
    } else {
      value = 29.7;
    }

    double referencePixel = distanceBetweenPoints();
    double width = widget.width;
    double height = widget.heigth;
    if (height > width) {
      double temp = height;
      height = width;
      width = temp;
    }
    final result = (value * referencePixel) / width;
    print(result);
    return result;
  }
                      ...
                      ...points.map((point) {
                        ...
                        return Positioned(
                          ...
                          child: GestureDetector(
                            ...
                            child: CircleAvatar(
                                radius: 3.5, backgroundColor: Colors.green),
                          ),
                        );
                      }).toList(),
                ...
                children: [
                  ...
                  IconButton(
                    icon: Icon(
                      Icons.check,
                      size: 40,
                      color: widget.image == null ? Colors.grey : Colors.green,
                    ),
                    onPressed: () {
                      final result = calculateDistance();
                      showDistancePopup(context, result);
                    },
                  ),
                ],
              ),
...
void showDistancePopup(BuildContext context, double distance) {
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return AlertDialog(
        title: Text("Mesafe Hesaplama"),
        content: Text(
          "İki nokta arasındaki mesafe: ${distance.toStringAsFixed(2)} cm",
